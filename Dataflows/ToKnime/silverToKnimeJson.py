...
and outputs one consolidated JSON file per ticker into the gold output directory.
"""
...
SILVER_PRICE_HISTORY_DIR = Path("../DataLake/Silver/price_history")
GOLD_OUTPUT_DIR          = Path("../DataLake/Knime/input/json")
...
def consolidate_ticker(ticker_path: Path, output_dir: Path, stocks_master: pd.DataFrame) -> None:
    ...
    sorts by date, and writes a single JSON file to output_dir.
    """
    ticker = ticker_path.name
    yearly_files = sorted(ticker_path.rglob("*.parquet"))
    ...
    # Convertir les dates en string ISO 8601 pour la sérialisation JSON
    if date_col:
        combined[date_col] = combined[date_col].dt.strftime("%Y-%m-%d")

    # Écriture en JSON (orient="records" → liste d'objets, un par ligne de données)
    output_path = output_dir / f"{ticker}.json"
    combined.to_json(output_path, orient="records", indent=2, force_ascii=False)
